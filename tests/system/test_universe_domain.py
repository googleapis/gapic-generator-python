...
def test_universe_domain_validation_fail(echo_with_localhost_universe):
    ...
    assert str(err.value) == "The configured universe domain (googleapis.com) does not match the universe domain found in the credentials (localhost:7469). If you haven't configured the universe domain explicitly, `googleapis.com` is the default."
