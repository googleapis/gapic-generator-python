...
def test_universe_domain_validation_fail(parametrized_echo, test_params):
    ...
    assert str(err.value) == f"The configured universe domain ({test_params.client_universe}) does not match the universe domain found in the credentials ({test_params.credential_universe}). If you haven't configured the universe domain explicitly, `googleapis.com` is the default."
