...
class DeleteLogRequest(proto.Message):
    ...
    Attributes:
        log_name (str):
            Required. The resource name of the log to
            delete:
...
class WriteLogEntriesRequest(proto.Message):
    ...
    Attributes:
        log_name (str):
            Optional. A default log resource name that is
            assigned to all log entries in `entries` that do
            not specify a value for `log_name`:
            ...
            * `folders/[FOLDER_ID]/logs/[LOG_ID]`
            ...
            The permission `logging.logEntries.create` is
            needed on each project, organization, billing
            account, or folder that is receiving new log
            entries, whether the resource is specified in
            `logName` or in an individual log entry.
        resource (google.api.monitored_resource_pb2.MonitoredResource):
            Optional. A default monitored resource object
            that is assigned to all log entries in `entries`
        ...
        labels (MutableMapping[str, str]):
            Optional. Default labels that are added to
            the `labels` field of all log entries in
            `entries`. If a log entry already has a label
            with the same key as a label in this parameter,
            then the log entry's label is not changed. See
            [LogEntry][google.logging.v2.LogEntry].
        entries (MutableSequence[google.cloud.logging_v2.types.LogEntry]):
            Required. The log entries to send to Logging.
            The order of log entries in this list does not
            matter. Values supplied in this method's
            `log_name`, `resource`, and `labels` fields are
            copied into those log entries in this list that
            do not include values for their corresponding
            fields. For more information, see the
            [LogEntry][google.logging.v2.LogEntry] type.

            If the `timestamp` or `insert_id` fields are
            missing in log entries, then this method
            supplies the current time or a unique
            identifier, respectively. The supplied values
            are chosen so that, among the log entries that
            did not supply their own values, the entries
            earlier in the list will sort before the entries
            later in the list. See the `entries.list`
            method.

            Log entries with timestamps that are more than
            the [logs retention
            period](https://cloud.google.com/logging/quotas)
            in the past or more than 24 hours in the future
            will not be available when calling
            `entries.list`. However, those log entries can
            ...
            include several log entries in this list, rather
            than calling this method for each individual log
            entry.
        partial_success (bool):
            Optional. Whether a batch's valid entries
            should be written even if some other entry
            failed due to a permanent error such as
            INVALID_ARGUMENT or PERMISSION_DENIED. If any
            entry failed, then the response status is the
            response status of one of the failed entries.
            The response will include error details in
            `WriteLogEntriesPartialErrors.log_entry_errors`
            keyed by the entries' zero-based index in the
            `entries`. Failed requests for which no entries
...
class WriteLogEntriesPartialErrors(proto.Message):
    ...
    Attributes:
        log_entry_errors (MutableMapping[int, google.rpc.status_pb2.Status]):
            ...
            is true, records the error status for entries
            that were not written due to a permanent error,
            keyed by the entry's zero-based index in
            `WriteLogEntriesRequest.entries`.

            Failed requests for which no entries are written
            will not include per-entry errors.
    """
...
class ListLogEntriesRequest(proto.Message):
    ...
    Attributes:
        resource_names (MutableSequence[str]):
            ...
            resources from which to retrieve log entries:
            ...
            *
            ...
            Projects listed in the `project_ids` field are
            added to this list. A maximum of 100 resources
            may be specified in a single request.
        filter (str):
            Optional. Only log entries that match the
            filter are returned.  An empty filter matches
            all log entries in the resources listed in
            `resource_names`. Referencing a parent resource
            that is not listed in `resource_names` will
            cause the filter to return no results. The
            maximum length of a filter is 20,000 characters.
        order_by (str):
            Optional. How the results should be sorted.
            Presently, the only permitted values are
            `"timestamp asc"` (default) and `"timestamp
            desc"`. The first option returns entries in
            order of increasing values of
            `LogEntry.timestamp` (oldest first), and the
            second option returns entries in order of
            decreasing timestamps (newest first).  Entries
            with equal timestamps are returned in order of
            their `insert_id` values.
        page_size (int):
            Optional. The maximum number of results to
            return from this request. Default is 50. If the
            value is negative or exceeds 1000, the request
        ...
        page_token (str):
            ...
            The values of other method parameters should be
...
class ListLogEntriesResponse(proto.Message):
    ...
    Attributes:
        entries (MutableSequence[google.cloud.logging_v2.types.LogEntry]):
            A list of log entries.  If `entries` is
            empty, `nextPageToken` may still be returned,
            indicating that more entries may exist.  See
        ...
        next_page_token (str):
            ...
            call this method again using the value of
            `nextPageToken` as `pageToken`.

            If a value for `next_page_token` appears and the
            `entries` field is empty, it means that the
            search found no log entries so far but it did
            not have time to search all the possible log
            entries.  Retry the method with this value for
            `page_token` to continue the search.
            Alternatively, consider speeding up the search
            by changing your filter to specify a single log
            name or resource type, or to narrow the time
            range of the search.
    """
    ...
    Attributes:
        ...
        resource_names (MutableSequence[str]):
            ...
            *  `folders/[FOLDER_ID]`
            ...
            The resource name in the `parent` field is added
            to this list.
        page_size (int):
            Optional. The maximum number of results to
            return from this request. Non-positive values
...
class TailLogEntriesRequest(proto.Message):
    ...
    Attributes:
        resource_names (MutableSequence[str]):
            Required. Name of a parent resource from
...
class TailLogEntriesResponse(proto.Message):
    ...
    Attributes:
        entries (MutableSequence[google.cloud.logging_v2.types.LogEntry]):
            A list of log entries. Each response in the
            stream will order entries with increasing values
            of `LogEntry.timestamp`. Ordering is not
    ...
    class SuppressionInfo(proto.Message):
        ...
        Attributes:
            ...
            suppressed_count (int):
                A lower bound on the count of entries omitted
                due to `reason`.
        """
        class Reason(proto.Enum):
            ...
            Values:
                REASON_UNSPECIFIED (0):
                    Unexpected default.
                RATE_LIMIT (1):
                    Indicates suppression occurred due to
                    relevant entries being received in excess of
                    rate limits. For quotas and limits, see [Logging
                    API quotas and
