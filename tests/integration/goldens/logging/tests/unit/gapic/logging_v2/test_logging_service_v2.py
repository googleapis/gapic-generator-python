...
def test__validate_universe_domain():
    ...
    client = LoggingServiceV2Client(transport=transports.LoggingServiceV2GrpcTransport(channel=channel))
    assert client._validate_universe_domain() == False

    # Test the case when there is a universe mismatch.
    client = LoggingServiceV2Client(credentials=_AnonymousCredentialsWithUniverseDomain(universe_domain="foo.com"))
    with pytest.raises(ValueError) as excinfo:
