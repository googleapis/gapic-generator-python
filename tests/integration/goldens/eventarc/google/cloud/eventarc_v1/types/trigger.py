...
class Trigger(proto.Message):
    ...
    Attributes:
        ...
        service_account (str):
            ...
            The `iam.serviceAccounts.actAs` permission must
            be granted on the service account to allow a
            principal to impersonate the service account.
            For more information, see the
            [Roles and
            permissions](/eventarc/docs/all-roles-permissions)
        ...
        event_data_content_type (str):
            Optional. EventDataContentType specifies the
            type of payload in MIME format that is expected
            from the CloudEvent data field. This is set to
...
class EventFilter(proto.Message):
    ...
    Attributes:
        attribute (str):
            ...
            attributes are supported for filtering. You can
            [retrieve a specific provider's supported event
        ...
        operator (str):
            ...
            matched. The allowed values are `path_pattern`
            and `match-path-pattern`. `path_pattern` is only
...
class Destination(proto.Message):
    ...
    Attributes:
        ...
        cloud_function (str):
            The Cloud Function resource name. Cloud
            Functions V1 and V2 are supported.
            Format:
            `projects/{project}/locations/{location}/functions/{function}`

            This is a read-only field. Creating Cloud
            Functions V1/V2 triggers is only supported via
            the Cloud Functions product. An error will be
...
class HttpEndpoint(proto.Message):
    ...
    Attributes:
        uri (str):
            ...
            Only HTTP and HTTPS protocols are supported. The
            host can be either a static IP addressable from
            the VPC specified by the network config, or an
            internal DNS hostname of the service resolvable
            via Cloud DNS.
    """
