...
def test__validate_universe_domain():
    ...
    client = CloudRedisClient(credentials=_AnonymousCredentialsWithUniverseDomain())
    ...
    client = CloudRedisClient(transport=transports.CloudRedisGrpcTransport(channel=channel))
    assert client._validate_universe_domain() == False

    # Test the case when there is a universe mismatch.
    client = CloudRedisClient(credentials=_AnonymousCredentialsWithUniverseDomain(universe_domain="foo.com"))
    with pytest.raises(ValueError) as excinfo:
