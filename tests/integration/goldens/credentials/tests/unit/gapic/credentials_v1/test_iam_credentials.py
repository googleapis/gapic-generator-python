...
@mock.patch.object(IAMCredentialsClient, "DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(IAMCredentialsClient))
@mock.patch.object(IAMCredentialsAsyncClient, "DEFAULT_ENDPOINT_TEMPLATE", modify_default_endpoint_template(IAMCredentialsAsyncClient))
def test__get_api_endpoint():
    api_override = "foo.com"
    universe_domain = "bar.com"
    mock_client_cert_source = mock.Mock()
    default_endpoint = IAMCredentialsClient.DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=IAMCredentialsClient.GOOGLE_DEFAULT_UNIVERSE)
    mock_endpoint = IAMCredentialsClient.DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)

    assert IAMCredentialsClient._get_api_endpoint(api_override, mock_client_cert_source, None, "always") == api_override
    assert IAMCredentialsClient._get_api_endpoint(None, mock_client_cert_source, None, "auto") == IAMCredentialsClient.DEFAULT_MTLS_ENDPOINT
    assert IAMCredentialsClient._get_api_endpoint(None, None, None, "auto") == default_endpoint
    assert IAMCredentialsClient._get_api_endpoint(None, None, None, "always") == IAMCredentialsClient.DEFAULT_MTLS_ENDPOINT
    assert IAMCredentialsClient._get_api_endpoint(None, mock_client_cert_source, None, "always") == IAMCredentialsClient.DEFAULT_MTLS_ENDPOINT
    assert IAMCredentialsClient._get_api_endpoint(None, None, universe_domain, "never") == mock_endpoint
    assert IAMCredentialsClient._get_api_endpoint(None, None, None, "never") == default_endpoint

    with pytest.raises(MutualTLSChannelError) as excinfo:
        IAMCredentialsClient._get_api_endpoint(None, mock_client_cert_source, universe_domain, "auto")
    assert str(excinfo.value) == "MTLS is not supported in any universe other than googleapis.com."

    with pytest.raises(ValueError) as excinfo:
        IAMCredentialsClient._get_api_endpoint(None, mock_client_cert_source, "", "auto")
...
def test__get_universe_domain():
    ...
    assert IAMCredentialsClient._get_universe_domain(None, None) == IAMCredentialsClient.GOOGLE_DEFAULT_UNIVERSE
