...
class Generator:
    ...
    def _generate_samples_and_manifest(
        ...
        if opts.autogen_snippets:
            spec_generator = samplegen.generate_sample_specs(api_schema, opts=opts)

        # Also process any handwritten sample specs
        handwritten_specs = samplegen.parse_handwritten_specs(self._sample_configs)

        for spec in itertools.chain(spec_generator, handwritten_specs):
        ...
        # TODO(busunkim): Re-enable manifest generation
