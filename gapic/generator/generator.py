...
class Generator:
    ...
    def get_response(
        ...
        for template_name in client_templates:
            ...
            output_files.update(
                self._render_template(template_name, api_schema=api_schema, opts=opts)
            )
    ...
    def _render_template(
        ...
        answer.update(self._get_file(template_name, api_schema=api_schema, opts=opts))
        return answer
    ...
    def _get_file(
        ...
        **context: Mapping,
        ...
        fn = self._get_filename(template_name, api_schema=api_schema, context=context,)
    ...
    def _get_filename(
        ...
        if context and "service" in context:
            filename = filename.replace("%service", context["service"].module_name,)

        # Replace the %proto variable if appliable.
        # In the cases of protos, we also honor subpackages.
        if context and "proto" in context:
            filename = filename.replace("%proto", context["proto"].module_name,)
